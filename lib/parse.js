...
// A title that is nothing but names ("natalie", "duffy nina joe") is a hangout.
function bareNameTitle(title) {
  ...
  return out.some((n) => n.length >= 3) ? out : [];
}
...
function parseJournal(text) {
  ...
  for (const rawLine of String(text).split(/\r?\n/)) {
    ...
    if (!people.length) {
      for (const n of bareNameTitle(title)) if (!people.includes(n)) people.push(n);
    }
    cur.activities.push({ time, title, location, rating, people, groupRef });
  }

  // Second pass: everyone named anywhere in the journal becomes a known name,
  // so bare mentions get credited too ("natalie", "Vedant Crib", "Cory bday")
  // even when the line has no "w/" clause.
  const known = new Map(); // lowercase -> display name
  for (const d of days) {
    for (const a of d.activities) {
      for (const n of a.people) known.set(n.toLowerCase(), n);
    }
  }
  for (const d of days) {
    for (const a of d.activities) {
