...
async function loadReports(email) {
  const db = await getDb();
  return db.collection('reports').findOne({ email });
}
...
async function saveReports(email, doc) {
  ...
  await db.collection('reports').updateOne(
    { email },
    { $set: { ...doc, email, updatedAt: new Date() } },
    { upsert: true }
  );
}

module.exports = { getDb, getUser, updateUser, loadCoachCache, saveCoachCache, loadReports, saveReports };
