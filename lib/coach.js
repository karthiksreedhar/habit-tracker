...
const WIDGET_IDS = [
  ...
  'people', 'places', 'sleep', 'plant', 'wins', 'focus', 'weekdays', 'activities', 'weather',
];
...
async function writeCache(email, cache) {
  // Merge with the stored doc right before writing: generations run for
  // minutes, and a concurrent write (user load vs. morning cron, daily vs.
  // weekly) must not clobber entries created meanwhile. Nothing ever deletes
  // an entry except pruning, so union-merge is safe.
  let base = null;
  try { base = await loadCoachCache(email); } catch {}
  const allDays = { ...((base && base.v === CACHE_VERSION && base.days) || {}), ...cache.days };
  const allWeeks = { ...((base && base.v === CACHE_VERSION && base.weeks) || {}), ...(cache.weeks || {}) };
  const days = {};
  for (const k of Object.keys(allDays).sort().slice(-14)) days[k] = allDays[k];
  const weeks = {};
  for (const k of Object.keys(allWeeks).sort().slice(-8)) weeks[k] = allWeeks[k];
...
async function generateCoach(email, { habits, journal, insights, widgetState = null, tz = null, force = false, forDate = null, constraintsBlock = '' }) {
  const now = nowInTz(tz);
  const todayKey = forDate || now.key;
  const prettyDay = forDate ? new Date(forDate + 'T12:00:00').toDateString() : now.pretty;
  ...
  const params = {
    ...
    messages: [{
      role: 'user',
      content: `Today is ${prettyDay}.${forDate ? ' (You are writing this day\'s card after the fact — write it as you would have that morning, using only what was knowable by then.)' : ''} Here is my data:\n\n${summary}${prevBlock}${widgetBlock}${constraintsBlock}\n\nGive me today's card.`,
    }],
  };
  ...
  // Suggested widgets must match the data that actually exists — sheet-only
  // or journal-sparse users get the habit-grid widgets, never empty cards.
  const hasHabits = habits.days.length > 0;
  const avail = {
    coach: true, 'weekly-coach': true, kpis: true,
    heatmap: hasHabits, 'habit-bars': hasHabits, wins: hasHabits, focus: hasHabits,
    weekdays: hasHabits || journal.some((d) => d.score !== null),
    rhythm: hasHabits || journal.some((d) => d.score !== null),
    today: journal.length > 0,
    impact: (insights.habitImpact || []).length > 0,
    people: (insights.people || []).length > 0,
    places: (insights.cities || []).length > 0,
    sleep: (insights.sleep || []).length > 0,
    plant: !!(insights.plant && insights.plant.total > 0),
    activities: (insights.activities || []).length > 0,
    weather: journal.some((d) => d.city),
  };
  const HABIT_FIRST = ['coach', 'kpis', 'heatmap', 'habit-bars', 'wins', 'focus', 'weekdays',
    'weekly-coach', 'rhythm', 'today', 'people', 'sleep', 'plant', 'activities', 'places', 'weather', 'impact'];
  if (!Array.isArray(result.widgets)) result.widgets = [];
  result.widgets = result.widgets.filter((w) => avail[w]);
  for (const w of HABIT_FIRST) {
    if (result.widgets.length >= 7) break;
    if (avail[w] && !result.widgets.includes(w)) result.widgets.push(w);
  }
  const entry = {
    result,
...
async function replaceTodo(email, { date, index, habits, journal, insights, tz = null, constraintsBlock = '' }) {
  const cache = await readCache(email);
  const entry = cache.days[date];
  if (!entry) throw new Error('No coach entry for ' + date);
...
async function generateWeekly(email, { habits, journal, insights, goalsBlock = '', force = false, constraintsBlock = '', tz = null }) {
  const now = new Date();
...
async function replaceWeeklyGoal(email, { week, index, habits, journal, insights, tz = null, constraintsBlock = '' }) {
  const cache = await readCache(email);
  cache.weeks = cache.weeks || {};
  const entry = cache.weeks[week];
  if (!entry) throw new Error('No weekly coach entry for ' + week);
