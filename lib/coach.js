...
Data notes: 🌱/🍃 = cannabis sessions ("No Chief" is the abstain habit — be matter-of-fact, no judgment). bedtime is minutes relative to midnight (75 = 1:15am, -30 = 11:30pm). Sample sizes may be small: treat patterns as signals, not laws.`;
...
Data notes: 🌱/🍃 = cannabis sessions ("No Chief" is the abstain habit — matter-of-fact, no judgment). bedtime is minutes relative to midnight (75 = 1:15am, -30 = 11:30pm). Small sample: patterns are signals, not laws.`;
...
async function generateCoach(email, { habits, journal, insights, widgetState = null, force = false }) {
  const todayKey = new Date().toISOString().slice(0, 10);
  const cache = await readCache(email);
  const existing = cache.days[todayKey];
  if (!force && existing) return dailyEntryToResponse(todayKey, existing, true);
  ...
  const params = {
    ...
    messages: [{
      role: 'user',
      content: `Today is ${new Date().toDateString()}. Here is my data:\n\n${summary}${prevBlock}${widgetBlock}\n\nGive me today's card.`,
    }],
  };
  ...
  return dailyEntryToResponse(todayKey, entry, false);
}
...
async function replaceTodo(email, { date, index, habits, journal, insights }) {
  const cache = await readCache(email);
  const entry = cache.days[date];
  if (!entry) throw new Error('No coach entry for ' + date);
...
async function generateWeekly(email, { habits, journal, insights, force = false }) {
  const now = new Date();
  const weekKey = isoWeekKey(now);
  const cache = await readCache(email);
  cache.weeks = cache.weeks || {};
  const existing = cache.weeks[weekKey];
  if (!force && existing) return weeklyEntryToResponse(weekKey, existing, true);
  if (!process.env.ANTHROPIC_API_KEY) {
    throw new Error('ANTHROPIC_API_KEY not set — add it to .env / Vercel env vars.');
  }
  ...
  const params = {
    ...
    messages: [{
      role: 'user',
      content: `Today is ${now.toDateString()} (ISO week ${weekKey}). Here is my data:\n\n${summary}${histBlock}${pwBlock}\n\nGive me this week's card.`,
    }],
  };
  ...
  await writeCache(email, cache);
  return weeklyEntryToResponse(weekKey, entry, false);
}
...
async function replaceWeeklyGoal(email, { week, index, habits, journal, insights }) {
  const cache = await readCache(email);
  cache.weeks = cache.weeks || {};
  const entry = cache.weeks[week];
  if (!entry) throw new Error('No weekly coach entry for ' + week);
