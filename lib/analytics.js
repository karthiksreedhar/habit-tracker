...
function computeActivities(days) {
  ...
  for (const d of days) {
    for (const a of d.activities) {
      if (a.flags.sleep || a.flags.wake) continue;
      // Unrated activities still COUNT — only the average skips them
  ...
  return [...map.values()]
    ...
    .sort((a, b) => (b.avgRating ?? -1) - (a.avgRating ?? -1) || b.n - a.n);
}
...
function buildInsights(habits, journal) {
  ...
  // --- people ---
  const people = computePeople(jDays);
  ...
  recent.peopleWindows = {
    week: windowOf(recentSlice, computePeople(recentSlice)),
    month: windowOf(monthSlice, computePeople(monthSlice)),
    all: windowOf(jDays, people),
  };
  recent.plantWindows = {
...
function activityKey(a) {
  ...
  if (/[\u{1F331}\u{1F343}]/u.test(raw)) return a.people.length ? '🌱 with people' : '🌱 solo';
  // 🍺/brews split by setting — drinking at home and drinking out are
  // different activities and rate differently
  if (/🍺|\bbrews?\b|\bbeers?\b/u.test(t)) {
    return /home|crib|porch|apt|apartment|house/.test(loc) ? '🍺 at home' : '🍺 out';
  }
  if (/\blifts?\b|\bgym\b/.test(t)) return 'Lift';
  if (/^uber|\blyft\b/.test(t)) return 'Uber';
  if (/booze|\bdrinks?\b|\bwine\b/.test(t)) return 'Drinks';
  if (/\bebike\b|\bbikes?\b/.test(t)) return 'Bike ride';

  // Fuzzy-normalize everything else so "Lunch W/ Joe", "lunch with sam!" and
  // "Lunch" all count as one activity:
  let base = t;
  const keepPerson = /^(call|meeting|chat|catch ?up|sync|facetime)\b/.test(base);
  if (!keepPerson) base = base.split(/\s+(?:w\/|w|with)\s+/)[0];       // cut people clause
  base = base
    .replace(/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}\u{FE0F}]/gu, ' ') // other emoji
    .replace(/\s*\([^)]*\)\s*/g, ' ')                                  // "(...)" notes
    .replace(/@.*$/, ' ')                                                 // inline locations
    .replace(/[^a-z0-9\s]/g, ' ')                                        // punctuation/case noise
    .replace(/\s+/g, ' ').trim();
  // light plural fold: softballs -> softball, chores -> chore
  base = base.split(' ')
    .map((w) => (w.length > 3 && w.endsWith('s') && !w.endsWith('ss') ? w.slice(0, -1) : w))
    .join(' ');
  if (!base) base = t.replace(/\s+/g, ' ').trim();
