...
function buildInsights(habits, journal) {
  ...
  const plantActs = jDays.flatMap((d) => d.activities.filter((a) => a.flags.plant));
  const lowDays = plantDaily.filter((d) => d.score !== null && d.count <= 1);
  const highDays = plantDaily.filter((d) => d.score !== null && d.count >= 2);
  const plant = {
    daily: plantDaily,
    total: plantActs.length,
    soloShare: plantActs.length
      ? round1((plantActs.filter((a) => a.people.length === 0).length / plantActs.length) * 100)
      : null,
    avgRating: round1(avg(plantActs.map((a) => a.rating))),
    avgScoreLowUse: round1(avg(lowDays.map((d) => d.score))),
    avgScoreHighUse: round1(avg(highDays.map((d) => d.score))),
    nLow: lowDays.length,
  ...
  for (const d of jDays) {
    for (const a of d.activities) {
      ...
      if (!actMap.has(key)) actMap.set(key, { title: key, n: 0, ratings: [] });
  ...
  return {
    ...
    sleep, plant, activities, weekdays, wins, focus, bestWeekday,
  };
}

// Group recurring activities under a stable label.
function activityKey(a) {
  const t = a.title.toLowerCase();
  if (/[\u{1F331}\u{1F343}]/u.test(a.title)) return a.people.length ? '🌱 with people' : '🌱 solo';
  if (/call with lydia/.test(t)) return 'Call with Lydia';
  if (/\blift\b/.test(t)) return 'Lift';
  if (/^uber/.test(t)) return 'Uber';
  if (/brews?\b|booze/.test(t)) return 'Drinks';
