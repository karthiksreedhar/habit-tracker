...
const REPORT_SCHEMA = {
  ...
  required: ['title', 'key_takeaways', 'snapshot', 'goal_progress', 'sections', 'experiments', 'open_questions', 'since_last_report'],
  properties: {
    title: {
      type: 'string',
    ...
    goal_progress: {
      type: 'array',
      description: 'One entry per stated goal, in the order given. Empty array when no goals are stated — never invent goals.',
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['goal', 'verdict', 'detail'],
        properties: {
          goal: { type: 'string', description: 'The goal in the user\'s own words, as given.' },
          verdict: { type: 'string', enum: ['on_track', 'close', 'slipping', 'no_signal'], description: 'Judged over this report period specifically, which may differ from the live status.' },
          detail: { type: 'string', description: 'One or two sentences on what the period did to this goal, citing a precomputed figure or logged event.' },
        },
      },
    },
    sections: {
...
The stats may include bigEvents: standout notes the user wrote in their journal between days (usually highlighted in their doc — emphasized:true — or written as bare lines). These are the period's actual milestones — offers, trips, breakups, launches. Treat them as first-class context: name them where relevant, connect the numbers to them (a score dip after a hard event, a streak break during a trip), and consider one for a key takeaway when it clearly shaped the period. Never moralize about them, and never invent events not listed.

sheetNotes are free-text daily comments from the user's habit spreadsheet — for users with no journal they are the only qualitative record. Use them like journal text: name what they describe, connect numbers to them, mine them for themes. They carry no scores or ratings — never convert a note into a number, and never present a note-derived hypothesis as a measured statistic.
...
When stated goals are provided: fill goal_progress with one honest entry per goal, judged over this period; let the goals shape which findings you lead with and make at least one experiment serve a goal that is slipping. Never soften a verdict to be encouraging, and never treat a goal as met without data behind it.
...
async function generateReport(email, { habits, journal, insights, goalsBlock = '', start = null, end = null, force = false }) {
  ...
  const params = {
    ...
    messages: [{
      ...
      content:
        ...
        prevBlock + goalsBlock +
        (goalsBlock ? '' : '\n\nNo goals are stated — return goal_progress as an empty array.'),
    }],
  };
  ...
  const next = [report, ...list].slice(0, 24); // saved-reports shelf
